@extends('layouts.app')
@section('title', 'Clock In — Heartstrings')
...
<div style="max-width:980px;margin:0 auto;padding:24px 20px 48px;">
        ...
        <div class="glass-strong panel fade-in delay-2" style="background:rgba(232,105,117,0.22);">
            ...
            @if($attendance && $attendance->clock_in)
                <div style="margin-bottom:18px;padding:14px 16px;background:rgba(22,163,74,0.12);border-radius:14px;border:1px solid rgba(22,163,74,0.25);">
                    ...
                    <div style="font-size:1.4rem;font-weight:700;color:#15803d;">{{ \Carbon\Carbon::parse($attendance->clock_in)->format('H:i') }}</div>
                    @if($attendance->clock_out)
                    <div style="font-size:0.8rem;color:#15803d;margin-top:4px;">Clocked out: {{ \Carbon\Carbon::parse($attendance->clock_out)->format('H:i') }}</div>
                    @endif
                </div>
            @else
            ...
            <div id="actionArea">
                ...
                <div style="display:flex;gap:10px;">
                    ...
                    <button id="clockBtn" onclick="captureAndSubmit()" class="btn-primary" style="flex:2;border-radius:12px;" disabled>
                        ...
                        {{ $attendance && $attendance->clock_in ? 'Take Photo to Clock Out' : 'Take Photo to Clock In' }}
                    </button>
                </div>
                @else
    ...
    <div class="glass panel fade-in delay-3" style="margin-top:20px;background:rgba(255,249,245,0.55);">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
            ...
            <a href="{{ route('profile.activity') }}" style="font-size:0.8rem;color:#BE0822;font-weight:500;text-decoration:none;">View all →</a>
        </div>
        <div style="overflow-x:auto;">
            <table class="data-table">
                ...
                <tbody>
                @forelse(auth()->user()->attendances()->orderBy('date','desc')->take(5)->get() as $a)
                <tr>
                    <td style="font-weight:500;">{{ $a->date->format('d M Y') }}</td>
                    <td>{{ $a->clock_in ?? '—' }}</td>
                    <td>{{ $a->clock_out ?? '—' }}</td>
