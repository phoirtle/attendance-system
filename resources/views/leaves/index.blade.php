@extends('layouts.app')
@section('title', 'My Leaves — Heartstrings')
...
<div style="max-width:980px;margin:0 auto;padding:24px 20px 48px;">
    ...
    @endif
    @if(session('error'))
    <div class="flash flash-error fade-in" style="margin-bottom:16px;">{{ session('error') }}</div>
    @endif
    ...
    <div class="fade-in delay-1" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
        <div style="display:flex;gap:16px;align-items:center;flex-wrap:wrap;">
            <div class="glass panel" style="padding:10px 18px;display:flex;gap:8px;align-items:center;">
                ...
                <span style="font-size:1rem;font-weight:700;color:#3d1a22;">{{ auth()->user()->remainingLeaveDays() }} / 12 days</span>
            </div>
        </div>
        <a href="{{ route('leaves.create') }}" class="btn-primary" style="text-decoration:none;display:inline-flex;align-items:center;gap:6px;border-radius:12px;padding:10px 20px;font-size:0.85rem;">
    ...
    <div class="glass-strong panel fade-in delay-2" style="background:rgba(255,249,245,0.60);padding:0;overflow:hidden;">
        <div style="overflow-x:auto;">
            <table class="data-table" style="min-width:700px;">
                ...
                <tr>
                    <td style="padding-left:24px;font-weight:500;text-transform:capitalize;">{{ $leave->type }}</td>
                    <td>{{ $leave->start_date->format('d M Y') }}</td>
                    <td>{{ $leave->end_date->format('d M Y') }}</td>
                    <td>{{ $leave->duration() }} days</td>
                    <td style="max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="{{ $leave->reason }}">{{ $leave->reason }}</td>
                    <td>
                    ...
                    <td style="padding-right:24px;">
                        ...
                        <form method="POST" action="{{ route('leaves.destroy', $leave) }}" style="display:inline;" onsubmit="return confirm('Cancel this leave request?');">
                            @csrf
                            @method('DELETE')
